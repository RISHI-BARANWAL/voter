...
router.get('/', authenticateToken, authorizeRoles('Super Admin', 'Admin'), async (req, res) => {
  try {
    ...
    if (user_id) match.user_id = new ObjectId(user_id);
    ...
    // Fetch logs with user full_name using $lookup
    const logsResult = await db.collection('auditlogs').aggregate([
      { $match: match },
      {
        $lookup: {
          from: 'users',
    ...
    const total = await db.collection('auditlogs').countDocuments(match);
